...
def change_active_screen(screen):
    ...
    global activeScreen
    activeScreen = screen #Save the screen
...
currTime = datetime.datetime.fromtimestamp(time()) #used for clock. Current time
lastRead = currTime - datetime.timedelta(seconds=(int(settings['interval'])*60)) #last time the TCs were read
...
activeScreen  = 'Main_Screen' #helps speed up the main loop by only checking code revelant to each screen
...
right = 0
maxTime = 0
stepSize = 1 #moves one data point
dataSlide = [1,zoom-1]
...
tab_group = [
    [sg.Tab("Main", wMain, key="Main_Screen",element_justification='c',background_color='#EEEEEE')],
    [sg.Tab("Settings", wSet, key="Settings_Screen",element_justification='c',background_color='#EEEEEE')],
    [sg.Tab("Logging", wLog, key="Log_Screen",element_justification='c',background_color='#EEEEEE')],
    [sg.Tab("Charge", wCharge, key="Charge_Screen",element_justification='c',background_color='#EEEEEE')],
]
...
window, errWin, chargeWin = sg.Window("Data Logger", layout, keep_on_top=True, location=(0, 0), element_justification='c',finalize=True), None, None
window.Maximize() #Make it full screen
window.set_icon(pngbase64=fireIcon) #Add the taskbar icon
    ...
    try:
        ...
        if activeScreen == 'Main_Screen':
            ...
            if event == 'goto_live':
                ...
                window['logInput'].update(visible = True) #Show input buttons for recording charges
                window['cDesc'].update(visible=False) #Hide charge description
                window['cRecord'].update(disabled=False) #Enable recording
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            #Recording a new charge
                ...
                if cCheck[0] and tCheck and dCheck:
                    ...
                    record_charge(str(values['TimeIn']).zfill(2) + '-' + values['ChargeIn'] + ' -- ' + values['TempIn'].zfill(4) + ' -- ' + currTime.strftime("%d-%b-%y"))
        ...
        elif activeScreen == 'Charge_Screen':
            
            #Display the old charge if the user has selected something
            if event == 'View' and values['cList']:
                change_active_screen('Log_Screen')
                ...
                chargeDisplay = True #Viewing a charge and not the live view
        ...
        elif activeScreen == 'Settings_Screen':
